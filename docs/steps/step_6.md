...
1. Автоподготовка ресурсов в `PREPARING` поддерживает сценарий on-demand:
   orchestrator может создавать недостающий объем под `target_users` на основе
   ресурсных деклараций сценария.
2. API `/create_resource` позволяет наполнять глобальный пул вручную.
